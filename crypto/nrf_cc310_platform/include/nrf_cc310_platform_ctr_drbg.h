 ...
 */
/**@file
 * @defgroup nrf_cc310_platform_ctr_drbg nrf_cc310_platform ctr_drbg generation APIs
 * @ingroup nrf_cc310_platform
 * @{
 * @brief The nrf_cc310_platform_ctr_drbg APIs provide PRNG seeded by TRNG in
 ...
 *        The generation of TRNG/PRNG data is using Arm CC310 hardware
 *        acceleration.
 *
 ...
 */
#ifndef NRF_CC310_PLATFORM_CTR_DRBG_H__
#define NRF_CC310_PLATFORM_CTR_DRBG_H__
 ...
 */
#define NRF_CC310_PLATFORM_ENTROPY_SIZE_WORDS (200)
 ...
 */
typedef struct nrf_cc310_platform_ctr_drbg_context_t
{
    uint32_t        is_initialized;									//!< Mask indicating if the ctr_drbg context has been initialized.
    uint32_t 		buffer[NRF_CC310_PLATFORM_ENTROPY_SIZE_WORDS];	//!< Internal buffer for the built-in entropy and ctrl_drbg contexts.
} nrf_cc310_platform_ctr_drbg_context_t;
 ...
 *                                      which may be zero.
 *
 * @retval NRF_CC310_PLATFORM_ERROR_PARAM_NULL Context was NULL.
 * @return 0 on success, otherwise a non-zero failure from mbedtls_ctrl_drbg_seed.
 */
int nrf_cc310_platform_ctr_drbg_init(
    nrf_cc310_platform_ctr_drbg_context_t * const context,
 ...
 */
int nrf_cc310_platform_ctr_drbg_free(
    nrf_cc310_platform_ctr_drbg_context_t * const context);
 ...
 * every call to @ref nrf_cc310_platform_ctr_drbg_get and
 * @ref nrf_cc310_platform_ctr_drbg_get_with_add. This leads to a higher power
 * draw and longer execution time.
 *
 * @note Before calling this api the context to must be initialized by calling
 *       @ref nrf_cc310_platform_ctr_drbg_init
 *
 ...
 */
int nrf_cc310_platform_ctr_drbg_set_pr(
    nrf_cc310_platform_ctr_drbg_context_t * const context,
    bool pr_enabled);
 ...
 */
int nrf_cc310_platform_ctr_drbg_set_reseed_interval(
    nrf_cc310_platform_ctr_drbg_context_t * const context, int interval);


/**@brief Function to do a manual reseed of ctr_drbg (using TRNG)
 *
 * @note Calling this API is optional as the APIs
 *       @ref nrf_cc310_platform_ctr_drbg_get and
 *       @ref nrf_cc310_platform_ctr_drbg_get_with_add functions will reseed
 ...
 */
int nrf_cc310_platform_ctr_drbg_reseed(
    nrf_cc310_platform_ctr_drbg_context_t * const context,
 ...
 * 16-byte key and reseed with TRNG using CC310 HW according to a reseed
 * interval.
 *
 ...
 *          random numbers are generated using Arm CC310 hardware acceleration.
 *
 * @note Before calling this api the context to must be initialized by calling
 *       @ref nrf_cc310_platform_ctr_drbg_init.
 *
 ...
 */
int nrf_cc310_platform_ctr_drbg_get_with_add(
    nrf_cc310_platform_ctr_drbg_context_t * const context,
    uint8_t *buffer,
    size_t len,
    size_t *olen,
 ...
 */
int nrf_cc310_platform_ctr_drbg_get(
    nrf_cc310_platform_ctr_drbg_context_t * const context,
    uint8_t *buffer,
    size_t length,
    size_t* olen);
...
#endif /* NRF_CC310_PLATFORM_CTR_DRBG_H__ */
